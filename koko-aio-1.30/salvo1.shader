...
vec3 clamp_01(vec3 source) {
	...
	// Input focus
	#define in_sharp_x     4.0           //10.0   0.5..10.0  Input signal x-sharpness 
	#define in_sharp_y     3.0            //3.0   2.0..5.0   Input signal y-sharpness
	                                       //too low values will "eat" scanlines.
                                           //too high values will cause scanline unphase with
	                                       //fs-uae autoscale.
	// Input glow
	#define in_glow_x     1.2            //2.0  Input signal blur/glow x-sharpness
	#define in_glow_y     1.2            //2.0  Input signal blur/glow y-sharpness
	#define in_glow_power 2.0            //2.0  Input signal glow strength
	#define in_glow_gamma 5.0            //5.0  The higher, the less the glow on dark colors	

	// Vertical RGB mask:       R     G     B
    #define m1 vec3 ( 1.0 , 0.0 , 0.0 )    //col 1
    #define m2 vec3 ( 0.0 , 1.0 , 0.0 )    //col 2
    #define m3 vec3 ( 0.0 , 0.0 , 1.0  )   //col 3

	// Horizontal darklines
	#define darkline_over_white 0.0				//0.0 0.0..1.0 how much darklines affect bright colors
	#define darkline_opacity	0.0				//0.0 0.0..1.0 global darklines power

	// Halo
	#define halo_x     0.5           //0.5..10.0  halo x-sharpness
	#define halo_y     0.5           //0.5..10.0  halo y-sharpness
	#define halo_power 2.0           //0.0..2.0   halo strength
	#define halo_gamma 5.0           //5.0   the higher, the less the halo on dark colors		
	
	// RGB gamma corrections
	#define color_correction vec3(0.46 ,0.45 , 0.44)    //(0.46,0.45,0.44) r,g,b, gamma correction.	
	#define GAMMA_COR    1.5                            //1.4   Gamma correction, the higher, the darker.
    ...
    void main(void) {
		...
		//Input focus
		pixel_out = pixel_glow(in_sharp_x,in_sharp_y,1.0,1.0);
		...
		vec3 glowed = pixel_glow(in_glow_x,in_glow_y,in_glow_power,in_glow_gamma);
		pixel_out +=glowed;
		//pixel_out=min(pixel_out,1.0); pixel_out=max(pixel_out,0.0) ; 
		
		//calcoliamo la luminosità prima del vmask.
		...
		//darklines
		//whiteness=0.5 trans=1.0-(0.0-(0.5*(1.0-0.0)))
		float darkline_transparency=1.0-(darkline_opacity-(whiteness*(1.0-darkline_over_white)));
		darkline_transparency=min(darkline_transparency,1.0);
		pixel_out = pixel_darklines_offset(pixel_out,4.0,darkline_transparency);	
		...
		//halo
		vec3 haloed = pixel_glow(halo_x,halo_y,halo_power,halo_gamma);
 		pixel_out +=haloed;
