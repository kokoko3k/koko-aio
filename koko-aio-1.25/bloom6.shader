...
//Sfoco il segnale in ingresso:
	...
	<fragment scale="0.25" filter="nearest"><![CDATA[
		uniform sampler2D rubyTexture;
		void main(void) {
			vec3 pixel = texture2D(rubyTexture, gl_TexCoord[0].xy).rgb; 
			gl_FragColor = vec4(pixel,1.0);
...
#define BLOOM_MULTIPLIER  3.5    //the more, the bloomer, try in range 3.0 - 5.0
...
#define saturation   2.0           //1.0    Saturation modifier, 1.0 is neutral saturation.
#define GAMMA_COR    0.4           //0.5   Gamma correction, the higher, the darker.
//                              R   G   B
