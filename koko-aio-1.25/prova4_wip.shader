...
#define DO_DARKLINES            0       //1      1 Enable darklines
...
#define DO_GLOW_OVER_VMASK       1       //0      1 draw glow 'over' VMASK pattern (need higher GAMMA_COR)  
#define DO_GLOW_OVER_SCRNL       1       //0      1 draw glow 'over' darklines
#define DO_BLOOM				 0		 //		  1 do fullscreen bloom
...
/* RGB mask and frame */
#define VMASK_HEIGHT              4      //4      When DO_DARKLINES_VOFFSET=1, draw one darkline (the frame) every # rows. Use even numbers.
#define VMASK_OVER_WHITE          1.0    //0.5   How much rgb vmask affects bright colors. Range [0.0 -> 1.0] 
#define DARKLN_OVER_WHITE         1.0   //0.05   How much darklines or vmask frame affect bright colors. Range [0.0 -> 1.0] 
                                                   //Lower levels simulates the glow of the image over the darkline
                                                

/* Darklines and RGB mask black frame */
#define DARKLINE_PERIOD         2       //2      When DO_DARKLINES_VOFFSET=0, draw one straight darkline every # rows. Use even numbers.
#define DARKLINE_HEIGHT         1       //2      straight darkline height

#define DARKLINES_TRANS         0.6     //0.0    Darklines or vmask frame alpha. Range [0.0 -> 1.0], ** 0.0 is the darker **



/* Glow, blur */
#define glowpix     0.5            //1.2  Blurriness and glow width , the less the blurrier
#define glowpixy    0.5            //1.2  Blurriness and glow height, the less the blurrier
#define glow         1.0           //1.2  Glow Strength, sane values from 1.0 to 1.3
#define glow_gamma   10.0           //2.0  the higher, the less the glow on dark colors
...
#define BLOOM_MULTIPLIER  1.0    //the more, the bloomer, try in range 3.0 - 5.0
#define BLOOM_SIGMA       3.0	  //lowering this, will make the the gaussian steeper, start with 10.0
#define BLOOM_GAMMA       2.2     //(1.0...+inf) the higher the less bark colors will bloom


/* Post effects: */

#define saturation   0.7           //1.0    Saturation modifier, 1.0 is neutral saturation.
 #define GAMMA_COR    1.5           //0.5   Gamma correction, the higher, the darker.
//                              R   G   B
#define color_correction vec3(0.46 ,0.45 , 0.44)      //(1.0,1.0,1.0) r,g,b, gamma correction.
...
vec3 pixel_bloomed(vec4 pixel_source) {
    ...
    for (int i = -BLOOM_KERNEL_SIZE; i <= BLOOM_KERNEL_SIZE; i++) {
		...
		aroundpx=texture2D(rubyTexture, texCoord);
			aroundpx = pow(aroundpx, vec4(BLOOM_GAMMA,BLOOM_GAMMA,BLOOM_GAMMA,BLOOM_GAMMA));
			color += aroundpx * blurWeight(float(i),bloom, BLOOM_SIGMA) ;	
		//if ( ( aroundpx.b+aroundpx.g+aroundpx.r )  >   threshold  )  { color += aroundpx * blurWeight(float(i),bloom, BLOOM_SIGMA) ;
