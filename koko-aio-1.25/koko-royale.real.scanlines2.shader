...
//Sfoco il segnale in ingresso:
	...
	<fragment scale="4.0" filter="linear"><![CDATA[
	...
	<fragment scale="0.25" filter="linear"><![CDATA[
		uniform sampler2D rubyTexture;
		void main(void) {
			vec3 pixel = texture2D(rubyTexture, gl_TexCoord[0].xy).rgb; 
			gl_FragColor = vec4(pixel,1.0);
...
//Upscale 4X per avere delle scanline più precise
	<fragment scale="4.0" filter="linear"><![CDATA[
		uniform sampler2D rubyTexture;
		void main(void) {
...
<fragment scale="1.0" filter="nearest"><![CDATA[
    ...
    #define SCAN_OVER_WHITE         0.3      //0.2  How much scanlines affect bright colors. Range [0.0 -> 1.0] 
...
/* Glow, blur */
#define glowpix     3.0            //1.2  Blurriness and glow width , the less the blurrier
#define glowpixy    3.0            //1.2  Blurriness and glow height, the less the blurrier
#define glow         3.0           //1.2  Glow Strength, sane values from 1.0 to 1.3



/* Real bloom */
#define BLOOM_KERNEL_SIZE 4      //bloom steps, the more the smoother, the slower, 4 could be enough.
#define BLOOM_WIDTH       0.005    //bloom width,  0.01 is a good starting point
#define BLOOM_HEIGHT      0.005    //bloom height, 0.01 is a good starting point
#define BLOOM_MULTIPLIER  2.0    //the more, the bloomer, try in range 3.0 - 5.0
...
#define GAMMA_COR    0.25           //0.5   Gamma correction, the higher, the darker.
//                              R   G   B
