...
/* Glow, blur */
#define glowpix     2.5            //1.2  Blurriness and glow width , the less the blurrier
#define glowpixy    2.5            //1.2  Blurriness and glow height, the less the blurrier
...
#define BLOOM_WIDTH       0.002    //bloom width,  0.01 is a good starting point
#define BLOOM_HEIGHT      0.002    //bloom height, 0.01 is a good starting point
#define BLOOM_MULTIPLIER  5.0    //the more, the bloomer, try in range 3.0 - 5.0
#define BLOOM_CLAMP		  1.0
#define BLOOM_SIGMA       10.0	  //lowering this, will make the the gaussian steeper, start with 10.0
#define BLOOM_THRESHOLD   1.0     //lower values limits bloom to darker colors, range 0.0 - 1.0
...
vec3 pixel_bloomed(vec4 pixel_source) {
    ...
    float source_brightness = max(max(pixel_source.r,pixel_source.g),pixel_source.b);

	
	//color=min(color,BLOOM_CLAMP);
	
	
    //bloom_mix=BLOOM_THRESHOLD-source_brightness;
    bloom_mix=1.0-pow(source_brightness, (BLOOM_THRESHOLD+1e-8));

    bloom_mix = max(bloom_mix,0.0);
    bloom_mix = min(bloom_mix,1.0);

					float bloom_brightness=max(max(color.r,color.g),color.b);
					bloom_mix=1.0-bloom_brightness;
	
	pixel_out=mix(pixel_source,color,bloom_mix);
	return vec3(pixel_out.r,pixel_out.g,pixel_out.b);
}
...
void main()
{
	...
	/* BLOOM */
	if ( DO_BLOOM == 1 ) {
	pixel_processed = pixel_bloomed(vec4(pixel_processed,1.0));
	}
