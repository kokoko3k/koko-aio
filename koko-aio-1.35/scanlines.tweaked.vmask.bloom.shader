...
<fragment scale="1.0" filter="linear"><![CDATA[
    #define DO_SCANLINES	true	//(true)	enable scanlines
...
<fragment filter="linear"><![CDATA[
	...
	#define DO_IN_GLOW		true			//(true)	Input signal glow
	#define IN_GLOW_ADD		1.0				//0.0		0.0-1.0 glow overflows from the scanlines and image becomes sharper
	#define in_glow_w		1.5				//(1.5)		Input signal blur/glow width tightness
	#define in_glow_h		1.0				//(1.5)		Input signal blur/glow height tightness
	#define in_glow_power	1.8             //(1.35)	Input signal glow strength
	#define in_glow_gamma	4.0				//(5.0)		1.0-9.0, The higher,  the less the glow on dark colors, use integers.
	...
	#define DO_DARKLINES			false	//(false)	Draw dark screenlines
	...
	#define DARKLINES_TRANSPARENCY	0.4		//(0.4)		0.0 to 1.0 from opaque to totally transparent
	...
	#define halo_w     1.5				//(0.5)		Halo width tightness
	#define halo_h     1.0				//(0.5)		Halo height tightness
	#define halo_power 2.0				//(2.0)		Halo strength
	#define halo_gamma 4.0				//(5.0)		1.0-9.0 The higher, the less the halo on dark colors, use integers.
	...
	#define bloom_gamma			3.0		//3.0		1.0-9.0 integer: restrict bloom to bright colors
	#define bloom_mix			0.22	//0.2		BLOOM final mix 
	#define bloom_over_scanline true	//true		Set it false to spare some gpu cycles if you disabled scanlines in the first fragment.
	...
	#define GAMMA					1.25		//1.5		Gamma correction, the higher, the darker.
...
vec3 blur(float Directions, float Quality, float Size) {
	...
	vec3 pixel_darklines(vec3 source,float darkline_every, float darkline_trans, bool do_offset) {
...
vec3 pixel_alternate(vec3 source) {
    ...
    void main(void) {
		vec3 pixel_out = texture2D(rubyTexture, gl_TexCoord[0].xy).rgb; 
		vec3 haloed;
		vec3 glowed;
		vec3 bloomed;
		...
		if (DO_DARKLINES) {
			pixel_out = pixel_darklines(pixel_out,DARKLINES_PERIOD,DARKLINES_TRANSPARENCY,DO_DARKLINES_VOFFSET);
		}
