...
<fragment scale="1.0" filter="nearest"><![CDATA[
    ...
    #define SCAN_OVER_WHITE         0.1      //0.2  How much scanlines affect bright colors. Range [0.0 -> 1.0] 
...
#define DO_SCREENLINES_VOFFSET    1       //1      1 Vertically offset even triads, only makes sense with DO_VMASK=1
...
#define DO_BLOOM_OVER_SCRNL       1       //0      1 draw bloom 'over' dark screenlines
...
#define VMASK_OVER_WHITE          0.6    //0.5   How much rgb vmask affects bright colors. Range [0.0 -> 1.0] 
#define SCRNLN_OVER_WHITE         1.0   //0.05   How much screenlines or vmask frame affect bright colors. Range [0.0 -> 1.0] 
...
#define SCREENLINES_TRANS         0.6     //0.0    Screenlines or vmask frame alpha. Range [0.0 -> 1.0], ** 0.0 is the darker **



/* Bloom, glow, blur */
#define bloompix     2.0            //1.2  Blurriness and glow width , the less the blurrier
#define bloompixy    3.0            //1.2  Blurriness and glow height, the less the blurrier
...
#define saturation   0.70           //1.0    Saturation modifier, 1.0 is neutral saturation.
#define GAMMA_COR_R    0.46           //0.5   Gamma correction, the higher, the darker.
#define GAMMA_COR_G    0.45           //0.5   Gamma correction, the higher, the darker.
#define GAMMA_COR_B    0.44           //0.5   Gamma correction, the higher, the darker.
...
vec3 whitemix(vec3 source, float screenline_transparency, float over_white, vec3 reference,float glowed_strength) {
    /* Non linear multiplication between "source" and "screenline_transparency"
       the more: over_white=1.0, the less the multiplication
       will be effective on bright colors.
	*/

	screenline_transparency=min(1.0,screenline_transparency);

    vec3 white_over_scanline = source -   ( (source -(source * screenline_transparency)) * (1.0 -reference) ) ;
    vec3 scanline_over_white = source * screenline_transparency;
...
vec3 mymix(vec3 source, float screenline_transparency, float over_white, vec3 reference,float glowed_strength) {
    /* Non linear mix between black and "source"
       the more over_white=1.0, the more the screenline on bright colors.
	   the more glowed_strength=1.0, the less the screenline
	   the more "screenline_transparency"=0,0, the less the screenline
	*/
	vec3 whitemixed=whitemix(source,screenline_transparency,over_white,reference,glowed_strength);
	//if (glowed_strength > 0.9) { return vec3(1.0,0.0,0.0);}
...
vec3 pixel_screenlines_offset(vec3 source, float glowed_strength) {
	...
	//zeroing glowed_strength has the effect of disabling the effwect of the glow over screenlines
	...
	if  (int(mod(line_2, screenline_every)) == 1) {
		if (int(mod(col_2, fScreenline_part_w_x2)) < iScreenline_part_w) {
			pixel_out =  mymix (source_clamped, SCREENLINES_TRANS, SCRNLN_OVER_WHITE,source,glowed_strength);
		}
...
vec3 pixel_screenlines_straight(vec3 source, float glowed_strength) {
	float screenline_every = float(SCREENLINE_HEIGHT);       
	float line_2 = gl_FragCoord.y;
	vec3 pixel_out=source;
	vec3 source_clamped = clamp_01(source) ;

	//zeroing glowed_strength has the effect of disabling the wffwect of the glow over screenlines
	if (DO_BLOOM_OVER_SCRNL == 0) { glowed_strength = 0.0; }
...
void main()
{
    ...
    if ( DO_SCREENLINES == 1 ) {
		if ( DO_SCREENLINES_VOFFSET == 1 ) {
			pixel_processed = pixel_screenlines_offset(pixel_processed,glowed_strength);
		} else {
			pixel_processed = pixel_screenlines_straight(pixel_processed,glowed_strength);
        }
    }

	/* GAMMA */
	pixel_processed = pow(pixel_processed, vec3(GAMMA_COR_R, GAMMA_COR_G, GAMMA_COR_B));
